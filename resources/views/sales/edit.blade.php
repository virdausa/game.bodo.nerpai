...
<div class="py-12">
    ...
    <form action="{{ route('sales.update', $sale->id) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="date" name="sale_date" class="bg-gray-100 w-full px-4 py-2 border rounded-md focus:ring focus:ring-blue-300 dark:bg-gray-700 dark:text-white" value="{{ $sale->sale_date }}" required >
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
            <div class="form-group">
                ...
                <select name="expedition_id" class="bg-gray-100 w-full px-4 py-2 border rounded-md focus:ring focus:ring-blue-300 dark:bg-gray-700 dark:text-white" required>
                    @foreach($couriers as $courier)
                        <option value="{{ $courier->id }}" {{ $sale->expedition_id == $courier->id ? 'selected' : '' }}>
                            {{ $courier->name }}
                        </option>
                    @endforeach
                </select>
